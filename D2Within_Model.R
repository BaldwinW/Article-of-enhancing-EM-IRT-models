...
RG_IRT_Model_MWti2PL <- function(
  ...
  ###Fit two dimensional 2PL IRT within item model####
  model_w <- mirt.model ('
          F1 = 1-100
          F2 = 51-100
          COV = F1*F2
  ...
  mtwopl2 <- mirt(data_replace_m, model_w, '2PL')
  
  est_itempars_mirt2 <- coef(mtwopl2, 
                             allpars = TRUE,
                             as.data.frame=TRUE)
  
  
  MB2 <- MDIFF(mtwopl2)[1:50]
  b_values <- data.frame(t(data.frame(rbind(b_true, MB2))))
  names(b_values) <- c("b_true", "b_est")
  write.csv(b_values, 
            file = paste0(Path_Save_Result, "/D2Within/b/b_", substr(csv_table, str_length(csv_table)-49, str_length(csv_table)-4), ".csv"), 
            row.names = F)
  
  MA2 <- MDISC(mtwopl2)[1:50]
  a_values <- data.frame(t(data.frame(rbind(a_true, MA2))))
  names(a_values) <- c("a_true", "a_est")
  write.csv(a_values, 
            file = paste0(Path_Save_Result, "/D2Within/a/a_", substr(csv_table, str_length(csv_table)-49, str_length(csv_table)-4), ".csv"), 
            row.names = F)
  
  ## Estimation of true ability and propensity of rapid guessing
  est_theta_mirt2 <- fscores(mtwopl2)
  
  theta_values <- data.frame(theta_true, est_theta_mirt2)
  write.csv(theta_values, 
            file = paste0(Path_Save_Result, "/D2Within/Theta/Theta_", substr(csv_table, str_length(csv_table)-49, str_length(csv_table)-4), ".csv"), 
            row.names = F)
